...
- **封装**：将数据和操作数据的方法组织在类中，通过访问修饰符隐藏内部实现，对外提供受控的访问接口。封装可以提高代码的安全性和可维护性。
- **继承**：子类通过 `extends` 继承父类，在复用父类已有功能的基础上进行扩展。Java 类只支持单继承，但接口支持多实现。
- **多态**：同一个父类或接口引用可以指向不同的子类对象，调用被重写的方法时，会根据对象的实际类型执行不同的实现。多态能够降低代码耦合，提高扩展性。

## a=a+b 和 a+=b的区别

`a += b` 大致等价于：`a = (a的类型) (a + b);`

区别在于，`a += b` 会隐式地将运算结果转换为 `a` 的类型，而 `a = a + b` 不会自动进行可能丢失精度的窄化转换。

例如：

```Java
short a = 1;
a += 2;       // 编译通过，相当于 a = (short) (a + 2)
a = a + 2;    // 编译错误，因为 a + 2 的结果是 int
```

如果 `a` 和 `b` 都是 `int`，对于普通变量来说，两者的运算结果通常相同：
...
![318](1-4、一些问题.assets/file-20260606135237921.jpg)
...
```
运行上面的代码，编译能过，运行报错：
**![754](1-4、一些问题.assets/file-20260606135237944.png)

原因：在Java中，Cat是Animal的子类，则Cat\[] 是 Animal\[]的子类，这样的性质被称为**协变**，==数组是协变的==。

### 泛型的不变性

![708](1-4、一些问题.assets/file-20260606135237938.png)
...
![660](1-4、一些问题.assets/file-20260606135237932.png)
...
![480](1-4、一些问题.assets/file-20260606135237935.png)
...
3. 不同类型的枚举不能互相比较或者赋值，因为类型不符。例如，不能给一个`Weekday`枚举类型的变量赋值为`Color`枚举类型的值：
...
- toString(): 和name()返回值相同。**但可被重写**。
- ordinal(): 顺序变了会跟着变，别用。**想将枚举值转为int，推荐用下面的代码**。`int n = Weekday.MON.ordinal(); // 1`
```java
// enum
public class Main {
    public static void main(String[] args) {
...
enum Weekday {
    ...
    public final int dayValue; // 可以不是final，但不推荐

    private Weekday(int dayValue) { // 注意：enum的构造方法是private
