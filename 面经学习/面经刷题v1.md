...
### 打破双亲委派的场景
1. **SPI机制**。JDBC的Driver接口定义在rt.jar里，由Bootstrap ClassLoader加载，但具体实现（比如MySQL驱动）在classpath下，Bootstrap ClassLoader加载不到。这时候用Thread.currentThread().getContextClassLoader()获取线程上下文类加载器来加载。（父层代码借助线程上下文类加载器反向发现应用层实现，是对双亲委派可见性限制的补充；AppClassLoader自身仍然遵循父优先规则。）
...
## 6. JDK1.8对HashMap改动
1. 红黑树
2. 哈希扰动简化：高16位和低16位异或
3. 扩容机制优化：不再逐个取模，元素要么留在原位，要么挪到 原位置+老数组长度
4. 头插法改尾插法

## 7. ConcurrentHashMap的get方法是否需要加锁？
不需要。数组元素读取用的是`Unsafe#getObjectVolatile`，Node节点的val和next指针也被volatile修饰，这三个关键点保证了整个读取链路的可见性。

get流程：
1. 通过hash定位数组下标，用getObjectVolatile读取数组槽位
2. 如果有hash冲突，沿着next指针遍历链表，next是volatile的
3. 找到目标节点后读取val，val也是volatile的

### 既然get不加锁，那put的时候get会不会读到中间状态？
JDK 8 的 ConcurrentHashMap 在桶树化后，并不是只靠红黑树指针查找。树桶由 TreeBin 管理，里面既有 root 红黑树根节点，也有 first 链表头。put 如果需要红黑树重平衡，会通过 lockRoot 设置写锁状态；而 get 发现有写锁或等待写锁时，不会遍历正在调整的红黑树，而是退化成遍历链表。所以不会因为红黑树旋转导致已经存在的节点查不到。并发插入的新节点是否被本次 get 看到，要看时序，不能保证。

## 8. 线程的生命周期在Java中是如何定义的？
1. NEW：线程对象刚new出来，还没调用start方法
2. RUNNABLE：调用start后进入该状态，注意这里包含了操作系统层面的就绪和运行两个状态，只要没被阻塞就算RUNNABLE
3. BLOCKED：线程想拿synchronized锁但没拿到，被挡在同步块外面等着
4. WAITING：调用了Object.wait、Thread.join或LockSupport.park，进入无限期等待，必须被别的线程显式唤醒
5. TIMED_WAITING：带超时时间的WAITING
6. TERMINATED：run方法执行完毕或者抛了未捕获异常，线程挂了

## 9. Java线程池有哪些拒绝策略？
1. AbortPolicy：直接抛RejectedExecutionException，默认策略。调用方能立即感知任务被拒，适合核心业务场景，比如订单支付必须让上游知道这单没处理。
2. CallerRunsPolicy：谁提交的任务谁执行。适合允许短暂阻塞调用的场景，比如后天日志异步写入。
3. DiscardOldestPolicy：踢掉队列里排队最久的任务，把当前任务塞进去
4. DiscardPolicy：静默丢弃，不抛异常也不执行。只适合那些丢了也无所谓的场景，比如埋点上报。
