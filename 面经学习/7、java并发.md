...
- 重锁原理：被作为锁使用的java对象的对象头（Mark Word）中会有一个值指向ObjectMonitor（Monitor的具体实现类）实例。ObjectMonitor包含字段：
	...
	- \_recursions：**记录器**。记录线程获得锁的次数。实现可重入锁
	- \_cxq：**竞争队列**。一个单向列表。当多线程竞争锁失败时，会通过CAS被放入`_cxq`的头部（处于阻塞状态）
	- \_EntryList：**阻塞队列**。这是一个双向链表。当 `_owner` 线程释放锁时，会根据策略（默认 QMode=0）将 `_cxq` 中的线程批量转移到 `_EntryList` 中，并唤醒其中的某个线程来竞争。这样做是为了降低并发 CAS 争抢 `_cxq` 带来的性能开销。
	- \_WaitSet：**等待队列**。如果线程 A 拿到了锁，但执行中调用了 `wait()` 方法，线程 A 就会释放锁（`_owner` 设为 NULL，`_count` 清零），并进入这个等待队列，状态变为 `WAITING`，直到被 `notify()` 唤醒后重新进入`_cxq`或 `_EntryList` 竞争锁。
