...
- ConcurrentHashMap：线程安全。空桶使用**CAS**插入，非空桶使用synchronized插入。在并发高时，使用数组统计size。使用volatile关键字让读操作在加锁时仍能读取。扩容时，若读取数据还未移到新table，直接读取；若读取到ForwardingNode节点，说明数据移到新table，此时去新table读取数据。如果是写操作碰到扩容，若写到ForwardingNode节点，则判断是否去协助扩容；若写到未被移到新table的数据，则直接在旧table中插入。
