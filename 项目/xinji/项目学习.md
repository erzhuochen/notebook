...
1. ai次数刷新不及时
2. 显示次数不够，但仍能分析
userId=2d37b4cfe2d329af0d4389b90fef3909
...
// @Component 将JwtAuthenticationFilter注册为一个Spring Bean，但它并没有自动将 Filter 放入 Servlet 容器的过滤链中。为了让 Filter 生效并参与请求处理，我们必须显式地将其注册到 FilterChain 中
...
 ```
 这个注解，会从SecurityContext中取出Authencation对象，然后再取出`Collection<GrantedAuthority> authorites`集合。然后比对当前用户是否有权限"readArtical"。实际上就是比对集合中是否有那个GrantedAuthority的getAuthority()方法返回的字符串与"radArtical"匹配。
...
# 二、日记
## 获取日记列表
![](项目学习.assets/file-20251231163225214.png)


# 三、redis
redis序列化器无法将response转换为json格式
```java
// 构建周报数据  
...
```
**解决**：
在 src/main/java/com/xinji/config/RedisConfig.java 中为 redisTemplate 注入 ObjectMapper，使用 new GenericJackson2JsonRedisSerializer(objectMapper)，从而复用 JacksonConfig 注册了 JavaTimeModule 的 Mapper，避免 LocalDate 序列化异常。**在注释中的代码为修改后代码**

## Redis - RedisTemplate及4种序列化方式深入解读

### 概述

使用Spring 提供的 Spring Data [Redis](https://cloud.tencent.com/product/crs?from_column=20065&from=20065) 操作redis 必然要使用Spring提供的模板类 RedisTemplate， 今天我们好好的看看这个模板类 。

---

### RedisTemplate

![](项目学习.assets/file-20260102091705702.png)

看看4个序列化相关的属性 ，主要是 用于 KEY 和 VALUE 的序列化 。 举个例子，比如说我们经常会将POJO [对象存储](https://cloud.tencent.com/product/cos?from_column=20065&from=20065)到 Redis 中，一般情况下会使用 JSON 方式序列化成字符串，存储到 Redis 中 。
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/d7d00fc959ab788e6521a074844e87f4.png)
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/5784b97087175c3ea87b7fbdc5e4fcb8.png)

RedisSerializer 接口的实现类 如下

![](https://ask.qcloudimg.com/http-save/yehe-8916337/9a2f0857fcf24833583849f0a60215b0.png)
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/f87c912f6f30d22d825120ef8b26e428.png)
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/5a344809f7d63d8913648a07aecf4919.png)

运行单元测试

![](https://ask.qcloudimg.com/http-save/yehe-8916337/e5ddf11dae102c16209153115424a70e.png)
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/c8c9354365d73ae4a3e77ed60facce69.png)
...
![](https://ask.qcloudimg.com/http-save/yehe-8916337/a49c5690abd715dfc18f4238a3713444.png)
