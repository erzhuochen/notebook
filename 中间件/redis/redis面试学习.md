...
```c
if(setnx(key, 1) == 1){
	// 问题1：设置锁和设置超时时间不保证原子性
	// 解决：使用lua脚本原子执行
	expire(key, 30) 
	// 问题2：业务执行到一半可能锁就过期了，导致其他进程拿到锁并执行业务
	// 解决：调高过期时间/增加守护进程，为将要过期但未释放的锁增加有效时间
	try {
	
	} finally {
		// 问题3：未检验锁是否还是自己的
		// 解决：在value中设置当前线程加锁的标识，使用lua校验
		del(key) 
	}
}
// 问题4：不可重入
// 解决：本地记录重录数（不推荐）/redis map记录重入次数。格式：[key:锁名] [field:线程标识] {value:重入次数}
...
### [](https://xiaomi-info.github.io/2019/12/17/redis-distributed-lock/#1-%E4%B8%BB%E5%A4%87%E5%88%87%E6%8D%A2 "1. 主备切换")1. 主备切换

为了保证 Redis 的可用性，一般采用主从方式部署。主从数据同步有异步和同步两种方式，Redis 将指令记录在本地内存 buffer 中，然后异步将 buffer 中的指令同步到从节点，从节点一边执行同步的指令流来达到和主节点一致的状态，一边向主节点反馈同步情况。

在包含主从模式的集群部署方式中，当主节点挂掉时，从节点会取而代之，但客户端无明显感知。当客户端 A 成功加锁，指令还未同步，此时主节点挂掉，从节点提升为主节点，新的主节点没有锁的数据，当客户端 B 加锁时就会成功。

![](https://xiaomi-info.github.io/2019/12/17/redis-distributed-lock/redis-lock-07.png)

### [](https://xiaomi-info.github.io/2019/12/17/redis-distributed-lock/#2-%E9%9B%86%E7%BE%A4%E8%84%91%E8%A3%82 "2. 集群脑裂")2. 集群脑裂

集群脑裂指因为网络问题，导致 Redis master 节点跟 slave 节点和 sentinel 集群处于不同的网络分区，因为 sentinel 集群无法感知到 master 的存在，所以将 slave 节点提升为 master 节点，此时存在两个不同的 master 节点。Redis Cluster 集群部署方式同理。

当不同的客户端连接不同的 master 节点时，两个客户端可以同时拥有同一把锁。如下：

![](https://xiaomi-info.github.io/2019/12/17/redis-distributed-lock/redis-lock-08.png)
### 面试问题
#### redis为什么快？
...
“Redis 采用单线程模型执行核心命令，是基于其性能瓶颈和架构设计的综合考量。 
首先，由于数据全在内存且底层数据结构极其高效（如哈希表、跳表），**CPU 根本不是瓶颈**，真正的瓶颈在于网络 I/O。 
其次，单线程模型**彻底抛弃了并发锁控制**（避免了类似 Java 中 AQS 或 CAS 的开销），并且**免疫了多线程上下文切换导致的 CPU 缓存失效**问题，同时天然保证了单个命令执行的**原子性**。 
至于网络 I/O 的瓶颈，Redis 早期通过底层的 **epoll** 实现了高效的事件分发；在 6.0 版本之后，更是在网络接入层引入了**多线程 I/O** 来并发处理 Socket 的读写，但核心内存操作依然坚持单线程，完美平衡了并发处理能力与系统复杂性。”
#### redis为什么要引入多线程
关于 Redis 的多线程，我们需要分两个阶段来看，它的演进是为了解决两类不同的性能瓶颈：
**第一类是系统调用和内存释放导致的阻塞瓶颈。** 比如删除一个百万级的大 Key，底层的 free() 操作极耗 CPU。所以在 **Redis 4.0** 引入了 UNLINK 机制和 BIO 后台线程。主线程只负责断开引用，释放内存的脏活交由后台线程异步处理，防止了主线程阻塞。

**第二类是网络 I/O 带来的吞吐量瓶颈。** 随着硬件发展，CPU 在处理系统调用（频繁的用户态与内核态切换）以及 RESP 协议解析上花费的时间，远超过内存数据操作。所以在 **Redis 6.0** 引入了多线程 I/O 模型。它让多个线程并发地去执行 Socket 的 read/write 和协议解析，但**核心的数据读写指令依然交由主线程串行执行**。

总结来说，Redis 引入多线程是对‘木桶效应’的精准修补，它把耗时的系统操作和网络 I/O 剥离出去，而坚决捍卫了核心计算单线程的无锁优势。”

#### 如何理解redis原子性操作原理？
Redis 的单条命令依靠单线程串行执行保证强原子性。它的 MULTI/EXEC 事务由于缺乏 Undo Log，在发生运行期错误时不支持回滚，因此无法提供关系型数据库级别的原子性保障。在实际的高并发架构中，我们统一放弃使用其自带的事务，而是通过 **Lua 脚本** 来实现复合操作的原子性，既保证了线程安全，又减少了网络 RTT 开销
