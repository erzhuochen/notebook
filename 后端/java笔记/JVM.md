...
| 收集器               | 主要特点              | 适合场景                |
| ----------------- | ----------------- | ------------------- |
| Serial            | 单线程，复制，STW        | 小程序、小堆              |
| ParNew            | 多线程，复制，STW        | 过去常配 CMS            |
| Parallel Scavenge | 吞吐量优先             | 后台任务、批处理            |
| Parallel Old      | 老年代吞吐优先           | 配 Parallel Scavenge |
| CMS               | 低停顿，标记-清除         | 老系统，已过时             |
| G1                | Region，预测停顿，服务端常用 | 现代后端服务              |
| ZGC               | 超低延迟              | 大内存、低延迟             |
| Shenandoah        | 低延迟，并发整理          | 低延迟场景               |
...
### 概述
基本数据类型由虚拟机预先定义，引用数据类型则需要进行**类的加载**。

从class文件到加载到内存中的类，到类卸载出内存：
**加载**（Loading） -> **链接** [ 验证（Verification） -> 准备（Preparation） -> 解析（Resolution）] -> **初始化**（Initialization） -> 使用（Using） -> 卸载（Unloading）

### 1. Loading
**所谓加载，简而言之就是将Java类的字节码文件加载到机器内存中，并在内存中构建出Java类的原型——类模板对象**。

加载类时，Java虚拟机必须完成一下3件事情：
- 通过类的全类名，获取类的二进制数据流
...
**final常量在怎么时候赋值**？
- 如果是 `static final` 的基本类型或字符串，它在类加载的**准备阶段**通过 `ConstantValue` 属性赋值，且极易触发调用方的**常量折叠**优化； 
- 如果是 `static final` 的引用类型，它在类加载的**初始化阶段**的 `<clinit>` 中赋值，由 JVM 保证线程安全； 
- 如果是实例 `final` 变量，在对象创建的 `<init>` 方法中赋值，并且 JVM 会在底层追加 **`StoreStore` 内存屏障**以防止对象逸出； 
- 如果是局部 `final` 变量，则是在运行时执行赋值，`final` 仅作为编译期的语义约束存在。”
...
### 3. 初始化
==初始化的重要工作是执行类的初始化方法：`clinit()`方法==。
clinit = 父类clinit + 类静态成员赋值语句 + static代码块语句